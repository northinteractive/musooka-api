...
class ToggleemailsubController {
    public $update;
    public $httpObj;
    public $response;
    ...
    public function __construct() {
        ...
        if(authorize::key($POST['user_id'],$POST['key'])) {
            $q = "UPDATE users SET unsub = IF(unsub=1, 0, 1) WHERE id = :id LIMIT 1";
            $sth = $DB->prepare($q);
            $sth->bindParam(":id",$POST['user_id']);
            
            if($sth->execute()) {
                $this->response['message'] = 'Successfully toggled email subscription value';
